...
int main(int argc, char* args[])
{
	...
	SDL_Event e;
	bool ai = false;
	std::cout << "Do you want to play against A.I.? (1/0)";
	std::cin >> ai;
	bool running = true;
	bool turn = 1; // 1 is O, 0 is X/ai
	if( ai )
	{
		...
		while( running )
		{
			if( turn )
			{
				while( SDL_PollEvent( &e ) > 0 && running && turn )
				{
					switch( e.type )
					{
						case SDL_QUIT:
			...
			{
				// we gotta secure that middle square
				if( board[ MIDDLE_CENTER ].isOccupied() == OCCUPIED_NOT )
				{
					std::cout << "AI prefers MIDDLE_CENTER" << std::endl;
					board[ MIDDLE_CENTER ].Occupy( turn );
					SDL_RenderCopy( gRenderer, gTextureX, NULL, &board[ MIDDLE_CENTER ].checker );
					SDL_RenderPresent( gRenderer );
					turn = !turn; // other p
					std::cout << "It is now your turn!" << std::endl;
				}
				...
				{
					int pref = abs(p_occupied - 2);
					while( board[ pref ].isOccupied() != OCCUPIED_NOT )
					{
						pref += 2;
					}
					std::cout << "AI prefers " << Enum_to_str( pref ) << std::endl;
					board[ pref ].Occupy( turn );
					SDL_RenderCopy( gRenderer, gTextureX, NULL, &board[ pref ].checker );
					SDL_RenderPresent( gRenderer );
					turn = !turn;
					std::cout << "It is now your turn!" << std::endl;
				}
